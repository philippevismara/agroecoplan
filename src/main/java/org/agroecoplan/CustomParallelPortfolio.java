 ...
 * </p>
 * <p>
 * The CustomParallelPortfolio resolution of a problem is made of four steps:
 *      <ol>
 ...
 *
 * <pre>
 * <code>CustomParallelPortfolio pares = new CustomParallelPortfolio();
 ...
 * @since 23/12/2015.
 */
public class CustomParallelPortfolio {
    ...
    public final List<AgroEcoPlanProblem> problems;
    ...
    public AgroEcoPlanProblem finderProblem;
    ...
    /**
     * Creates a new CustomParallelPortfolio
     ...
     */
    public CustomParallelPortfolio(boolean searchAutoConf) {
     ...
     */
    public CustomParallelPortfolio() {
        this(true);
    }
     ...
     * <b>Important:</b>
     *  <ul>
     *      <li>the populating process is not managed by this CustomParallelPortfolio
     ...
     * </p>
     *
     * @param problem a model to add
     */
    public void addModel(AgroEcoPlanProblem problem) {
        problems.add(problem);
        addModel(problem.getModel(), true);
    }
     ...
     * <b>Important:</b>
     *  <ul>
     *      <li>the populating process is not managed by this {@code CustomParallelPortfolio}
     *  and should be done externally, with a dedicated method for example.
     *  </li>
     *  <li>
     *      when dealing with optimization problems, the objective variables <b>HAVE</b> to be declared eagerly with
    ...
    /**
     * @return the (mutable!) list of models used in this CustomParallelPortfolio
     */
    ...
    public void prepare() {
        ...
        for (int i = 0; i < models.size(); i++) {
            ...
            AgroEcoPlanProblem p = problems.get(i);
            s.addStopCriterion(() -> getSolverTerminated().get());
            s.plugMonitor((IMonitorSolution) () -> updateFromSolution(s.getModel(), p));
            if (searchAutoConf) {
                configureModel(i);
            }
        }
    ...
    private synchronized void updateFromSolution(Model m, AgroEcoPlanProblem p) {
    ...
    private void check() {
        if (models.size() == 0) {
            throw new SolverException("No model found in the CustomParallelPortfolio.");
        }
                ...
                for (Constraint c : models.get(0).getCstrs()) {
                    if (c instanceof RealConstraint) {
                        throw new UnsupportedOperationException("" +
                                "Ibex is not multithread safe, CustomParallelPortfolio cannot be used");
                    }
                }
            }
        }
